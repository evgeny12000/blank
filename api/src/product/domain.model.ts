...
export class Domain extends Model<DomainsAttributes, DomainsAttributes> implements DomainsAttributes {
    @Column({ primaryKey: true, autoIncrement: true, type: DataType.INTEGER })
    @Index({ name: "PRIMARY", using: "BTREE", order: "ASC", unique: true })
    id?: number;
    @Column({ field: "domain_name", allowNull: true, type: DataType.STRING(255) })
    @Index({ name: "domain_name", using: "BTREE", order: "ASC", unique: true })
    domainName?: string;
    @Column({ field: "lock_key", allowNull: true, type: DataType.STRING(50) })
    @Index({ name: "lock_key", using: "BTREE", order: "ASC", unique: true })
    lockKey?: string;
    @Column({ field: "worker_id", allowNull: true, type: DataType.STRING(50) })
    workerId?: string;
    @Column({ field: "sync_status", allowNull: true, type: DataType.ENUM('ready_to_start','inprogress','done'), defaultValue: "ready_to_start" })
    syncStatus?: string;
